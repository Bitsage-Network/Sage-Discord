...
interface EditConditionDialogProps {
  ...
  onUpdate: (conditionIndex: number, updates: Partial<Condition>) => void
  onClose: () => void
  guildId: string
}
...
export function EditConditionDialog({
  ...
  const [formData, setFormData] = useState<any>(condition.condition_data || {})
  ...
  useEffect(() => {
    const typeChanged = conditionType !== condition.condition_type
    const labelChanged = (label || "") !== (condition.label || "")
    const dataChanged = JSON.stringify(formData) !== JSON.stringify(condition.condition_data || {})

    setHasChanges(typeChanged || labelChanged || dataChanged)
  }, [conditionType, label, formData, condition])
  ...
  const handleUpdate = () => {
    ...
    onUpdate(conditionIndex, updates)
    onClose()
  }
      ...
      >
        ...
        <div className="border-b border-gray-700 px-6 py-4">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-white">Edit Condition</h2>
              <p className="text-sm text-gray-400 mt-1">
                Modify the requirement that users must meet
              </p>
            </div>
            {hasChanges && (
              ...
              {conditionTypes.map((type) => (
                <button
                  ...
                  onClick={() => {
                    ...
                    // Only reset formData if changing type
                    if (type.value !== condition.condition_type) {
                      setFormData({})
                    }
                  }}
                  className={`
